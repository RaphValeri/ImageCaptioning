...
class CaptioningModel(nn.Module):
    """
    Class of the captioning model combining the visual encoder of the CLIP model and the language decoder of the LLaMA
    model
    """
    def __init__(self, ckpt_dir: str, tokenizer_path: str, local_rank: int, world_size: int, max_seq_len: int, max_batch_size: int, nb_ca=1):
        """
        Constructor of the captioning model
        @param ckpt_dir:
        @param tokenizer_path: path to the LLaMA tokenizer file
        @param local_rank: local rank of the GPU
        @param world_size: world size for the number of GPUs
        @param max_seq_len: maximum length of the sequence generated by the LLaMA model
    ...
    def generateCap(self, img, temperature=0.0, max_gen_len=30, top_p=0.95, verbose=False, return_tokens=False):
        ...
        @param temperature: temperature value to use during inference
        @param max_gen_len: maximum length of the generated caption
        @param top_p: cumulative probability to use during sampling
        @param verbose: boolean to indicate if intermediate information need to be printed
        @return: the caption
        """
        prompts = [""]
        bsz = len(prompts)
...
class CrossAttention(nn.Module):
    """
    Implementation of the cross-attention inspired by the initial implementation of the self-attention of the LLaMA model
    """
    ...
    def forward(self, x: torch.Tensor, x_img: torch.Tensor, start_pos : int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        ...
        # print('scores shape : ', scores.shape)
        self.scores_att = F.softmax(scores.float(), dim=2).type_as(xq)
        # print('After softmax')
        # print('scores shape : ', scores.shape)
